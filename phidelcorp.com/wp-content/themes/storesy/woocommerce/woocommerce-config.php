...
if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.3', '>=' ) ) {
	add_filter( 'woocommerce_add_to_cart_fragments', 'cg_cart_link_fragment' );
} else {
	add_filter( 'add_to_cart_fragments', 'cg_cart_link_fragment' );
}
...
// Size Guide action
// add_action( 'woocommerce_single_product_summary', 'cg_woocommerce_size_guide', 51 );
...
if ( !function_exists( 'cg_woocommerce_size_guide' ) ) {
	...
	function cg_woocommerce_size_guide() {
		...
		<?php
		if ( $size_guide_title ) {
			$cg_options['product_size_guide']['url'] = $cg_protocol . str_replace( array( 'http:', 'https:' ), '', $cg_options['product_size_guide']['url'] );
			?>
	...
	function cg_product_toggle() {
		...
		if ( $product_layout == 'grid-layout' ):
			...
			<div class="view-switcher clearfix">
				<label><?php esc_html_e( 'View as:', 'storesy' ); ?></label>
				<div class="toggleGrid" data-tooltip="<?php esc_html_e( 'Grid View', 'storesy' ); ?>"><i class="fa fa-th fa-2x"></i></div>
				<div class="toggleList" data-tooltip="<?php esc_html_e( 'List View', 'storesy' ); ?>"><i class="fa fa-list fa-2x"></i></div>
			</div>
		<?php elseif ( $product_layout == 'list-layout' ): ?> 
			<div class="view-switcher clearfix">
				<label><?php esc_html_e( 'View as:', 'storesy' ); ?></label>
				<div class="toggleList" data-tooltip="<?php esc_html_e( 'Grid View', 'storesy' ); ?>"><i class="fa fa-list fa-2x"></i></div>
				<div class="toggleGrid" data-tooltip="<?php esc_html_e( 'List View', 'storesy' ); ?>"><i class="fa fa-th fa-2x"></i></div>
			</div>
		<?php endif; ?> 
 ...
 */
add_filter( 'woocommerce_product_tabs', 'cg_woo_remove_product_tabs', 98 );

function cg_woo_remove_product_tabs( $tabs ) {
	unset( $tabs['reviews'] ); // Remove the reviews tab
	return $tabs;
}

/**
 * Remove on single product tab "Product Description" heading since it already says it on the tab.
 */
add_filter( 'woocommerce_product_description_heading', 'cg_product_description_heading' );

function cg_product_description_heading() {
	return '';
}

/**
 * Remove on single product tab "Additional Information" since it already says it on the tab.
 */
add_filter( 'woocommerce_product_additional_information_heading', 'cg_product_additional_information_heading' );

function cg_product_additional_information_heading() {
	return '';
}
...
function cg_woocommerce_cart_dropdown() {
	...
	<ul class="tiny-cart">
		<li>
			<a class="cart_dropdown_link cart-parent" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php esc_html_e( 'View your shopping cart', 'storesy' ); ?>">
				<div class="cg-header-cart-icon-wrap">


				</div>
				<span class='cart_subtotal'><?php echo WC()->cart->cart_contents_count; ?> <span class="dash">/</span> <?php echo wp_kses_post( $woocommerce->cart->get_cart_total() ); ?></span> 
			</a>
	<?php
	...
	if ( sizeof( $woocommerce->cart->cart_contents ) > 0 ) : foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) :
			...
			if ( $_product->exists() && $cart_item['quantity'] > 0 ) :
				...
				echo esc_url( $_product->get_image() );
				...
				?>
								...
								<div class="col-lg-2">
									<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s">x</a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'woocommerce' ) ), $cart_item_key ); ?>
								</div>
							</div>
						</div>
					</li>
	...
	else: echo '<li class="empty">' . __( 'No products in the cart.', 'storesy' ) . '</li>';
	endif;
	if ( sizeof( $woocommerce->cart->cart_contents ) > 0 ) :
		echo '<li class="total"><strong>';

		if ( get_option( 'js_prices_include_tax' ) == 'yes' ) :
			esc_html_e( 'Total', 'storesy' );
		else :
			esc_html_e( 'Subtotal', 'storesy' );
		endif;
		...
		echo '<li class="buttons"><a href="' . $woocommerce->cart->get_cart_url() . '" class="button">' . __( 'View Cart', 'storesy' ) . '</a> <a href="' . $woocommerce->cart->get_checkout_url() . '" class="button checkout">' . __( 'Checkout', 'storesy' ) . '</a></li>';
	endif;
...
// Handle cart in header fragment for ajax add to cart
	...
	function next_post_link_product( $format = '%link &raquo;', $link = '%title', $in_same_cat = false,
								  $excluded_categories = '' ) {
		adjacent_post_link_product( $format, $link, $in_same_cat, $excluded_categories, false );
	}

	function previous_post_link_product( $format = '&laquo; %link', $link = '%title', $in_same_cat = false,
									  $excluded_categories = '' ) {
	...
	function adjacent_post_link_product( $format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true ) {
		...
		if ( !$post ) {
			...
			if ( empty( $post->post_title ) )
				$title = $previous ? __( 'Previous Post', 'storesy' ) : __( 'Next Post', 'storesy' );
			...
			$string	 = '<a href="' . get_permalink( $post ) . '" rel="' . $rel . '" class="';
			$inlink	 = str_replace( '%title', $title, $link );
			$inlink	 = $string . $inlink . '">' . '<div>' . get_the_post_thumbnail( $post->ID, apply_filters( 'single_product_small_thumbnail_size', 'shop_thumbnail' ) ) . '</div></a>';
			...
			if ( !empty( $excluded_categories ) ) {
				if ( !is_array( $excluded_categories ) ) {
					...
					if ( strpos( $excluded_categories, ' and ' ) !== false ) {
						_deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded categories.', 'storesy' ), "'and'" ) );
