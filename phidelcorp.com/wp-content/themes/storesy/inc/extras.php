...
function cg_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'storesy' ), max( $paged, $page ) );
...
function cg_page_title_callback() {
	...
	<?php if ( $cg_show_page_title !== 'Hide' ) { ?>
		<div class="header-wrapper">
			<div class="overlay" <?php echo wp_kses_post( $cg_page_title_background_color_style ); ?>></div> 
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<header class="entry-header">
							...
							?>
							<h1 class="cg-page-title" <?php echo wp_kses_post( $cg_page_title_font_color_style ); ?>><?php the_title(); ?></h1>
						</header>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-lg-12">
					<header class="entry-header">
						...
						<h1 class="cg-page-title" <?php echo wp_kses_post( $cg_page_title_font_color_style ); ?>><?php echo wp_kses_post( $cg_blog_page_title ); ?></h1>
					</header>
				</div>
			</div>
		</div>
	...
	if ( is_wc_active() ) {
		if ( is_product_category() ) {
			...
			if ( isset( $queried_object->taxonomy ) ) {
				...
				$alt_text	 = $queried_object->name;
			}

			if ( function_exists( 'get_field' ) ) {
				$cat_banner = get_field( 'product_category_banner', $taxonomy . '_' . $term_id );
				if ( !empty( $cat_banner ) ) {
					$cg_woo_cat_banner_src	 = wp_get_attachment_image_src( $cat_banner, 'cg-product-category-banner' ); // returns an array
					$cg_woo_cat_banner_image = wp_get_attachment_image( $cat_banner, 'cg-product-category-banner', false, array( 'class' => 'product-category-banner' ) );

					if ( !empty( $cg_woo_cat_banner_src ) ) {
						$cg_banner_html .= '' . $cg_woo_cat_banner_src[0] . '';
						return $cg_banner_html;
					}
				} else {
					return false;
				}
...
function cg_get_woo_cat_intro() {
	...
	$cg_queried_object = get_queried_object();

	if ( isset( $cg_queried_object->term_id ) ) {
		$cat_id		 = $cg_queried_object->term_id;
		$cat_desc	 = term_description( $cat_id, 'product_cat' );
	}


	$cg_woo_cat_banner = cg_woo_cat_banner();

	// if img and cat exists
	if ( ( $cg_woo_cat_banner ) && ( $cat_desc ) ) {

		echo '
				<div class="cg-category-wrapper with-image" style="background-image:url(' . cg_woo_cat_banner() . ')">
			  		...
			  		<div class="cg-category-inner">
						<div class="container">
						    <div class="row">
								...
								<h1 class="cg-page-title">' . woocommerce_page_title( false ) . '</h1>
								' . $cat_desc . ' 
								</div>
							</div>
						</div>
